...
applying_rmwagen <- function (info_station, TEMPERATURE_MAX, TEMPERATURE_MIN, PRECIPITATION, n_GPCA_iter = 10, n_GPCA_iteration_residuals =10, lag=2, p_prec = 3, p_test =2,menu, manual, choose_station)
{
    if (manual == 1)
    {
...
applying_rmwagen_ALL <- function (info_station, TEMPERATURE_MAX, TEMPERATURE_MIN, PRECIPITATION, n_GPCA_iter = 10, n_GPCA_iteration_residuals =10, lag=2, p_prec = 3, p_test =2,menu, choose_station)
{
  #Read Station 
  stations <- read.csv("./Results/Results_DailyControl.csv", header=TRUE)
  stations$Station_Name <- unique(stations$Station_Name)
